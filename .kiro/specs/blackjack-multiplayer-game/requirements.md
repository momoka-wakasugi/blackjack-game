...
**ユーザーストーリー:** プレイヤーとして、初心者でも簡単にゲームを理解でき、快適にプレイできるインターフェースを使いたい。

#### 受入基準

1. WHEN プレイヤーがゲームルームに入室する THEN THE System SHALL 遊び方を説明するポップアップを自動表示する
2. WHEN 遊び方ポップアップが表示される THEN THE System SHALL ゲームの目的、カードの数え方、ゲームの流れ、アクション、配当を日本語で説明する
3. WHEN プレイヤーが遊び方ポップアップを閉じたい THEN THE System SHALL 閉じるボタン（×）または「わかりました」ボタンで閉じられるようにする
4. WHEN プレイヤーがヒットボタンにマウスオーバーする THEN THE System SHALL 「カードをもう1枚引きます」というツールチップを表示する
5. WHEN プレイヤーがスタンドボタンにマウスオーバーする THEN THE System SHALL 「カードを引かずに終了します」というツールチップを表示する
6. WHEN プレイヤーがルームから退出したい THEN THE System SHALL ヘッダーに退出ボタンを表示する
7. WHEN プレイヤーが退出ボタンをクリックする THEN THE System SHALL 確認ダイアログを表示し、確認後にルーム選択画面に戻る
8. WHEN プレイヤーがルームから退出する THEN THE System SHALL プレイヤーの残高などの情報を保持しない
9. WHEN プレイヤーが1360x768の画面サイズでアクセスする THEN THE System SHALL スクロールなしですべての操作要素が表示されるレイアウトを提供する
10. THE System SHALL ゲーム画面のレイアウトを1360x768の画面サイズに最適化する